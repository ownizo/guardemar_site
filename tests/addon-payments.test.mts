...
test('gross EUR parsing and included 23% VAT use integer minor units',()=>{
 for(const [eur,gross,net,vat]of [['98.40',9840,8000,1840],['18.45',1845,1500,345],['30.75',3075,2500,575],['0.01',1,1,0]]as const){assert.equal(euroMinorUnits(eur),gross);assert.deepEqual(includedVat(gross),{gross,net,vat,rate:23})}
...
test('one canonical catalogue has 13 admin categories and 12 customer services',()=>{assert.equal(addonServiceCatalogue.length,13);assert.equal(optionalServices.length,12);assert.ok(!optionalServices.some(s=>s.id==='external-provider'));assert.equal(addonServiceCatalogue.at(-1)?.adminOnly,true)})
...
test('Admin confirmation UX has unchecked monthly default, explicit VAT label and no return activation',()=>{
 const ui=readFileSync('src/components/portal/addon-payment-form.tsx','utf8');assert.ok(ui.includes('useState(false)'));assert.ok(ui.includes('Monthly recurring payment'));assert.ok(ui.includes('FINAL AMOUNT (VAT INCLUDED)'));assert.ok(ui.includes('Review payment'));assert.ok(ui.includes('SEND SUBSCRIPTION LINK'));assert.ok(ui.includes('confirmed: true'));const route=readFileSync('src/routes/portal.add-on-payment-return.tsx','utf8');assert.ok(!route.includes('rpc('));assert.ok(!route.includes('payment_status:'));assert.ok(!ui.includes('Refund'))
})
