...
test('operational verification tools create no charges and print no credentials', async () => {
  ...
  const taxRateProvisioning = await source('../src/lib/server/stripe-vat-rate.ts')
  ...
  assert.match(taxRateSetup, /provisionApprovedLiveVatRate/)
  assert.match(taxRateProvisioning, /acct_1QjMaJHFqsWIut8W/)
  assert.match(taxRateProvisioning, /\/tax_rates\?\$\{query\}/)
  assert.match(taxRateProvisioning, /matches\.length > 1/)
  assert.match(taxRateProvisioning, /'\/tax_rates'/)
  assert.doesNotMatch(`${taxRateSetup}\n${taxRateProvisioning}`, /\/checkout\/sessions|\/payment_intents|stripeRequest[^\n]+['"]\/(customers|invoices|subscriptions)['"]/)
