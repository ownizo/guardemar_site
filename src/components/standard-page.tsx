import { AssessmentCta, Breadcrumbs, ContentSection, FAQSection, PageHero } from '@/components/site'
import { legalPages, standardPages } from '@/config/pages'
...
export function StandardPageView({ pageKey, legal = false }: { pageKey: string; legal?: boolean }) {
  ...
  return <>{!legal&&<script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(serviceSchema)}}/>}<Breadcrumbs items={[{ label: 'Home', to: '/' }, { label: page.title }]} /><PageHero eyebrow={page.eyebrow} title={page.title} text={page.description} aside={page.aside} /><div className="section shell content-wrap">{page.sections.map((section) => <ContentSection title={section.title} key={section.title}>{section.paragraphs?.map((p) => <p key={p}>{p}</p>)}{section.bullets && <ul>{section.bullets.map((item) => <li key={item}>{item}</li>)}</ul>}</ContentSection>)}</div>{pageKey==='property-care'&&<FAQSection/>}{!legal && <AssessmentCta />}</>
}
