...
import { addonStatusLabels, priceDisclaimer, requestConfirmation, shoppingPaymentMessage, addonPaymentStatusLabels, type AddonRequest, type AddonPayment, type ShoppingItem } from '@/lib/portal/addons'
...
export function ServiceRequests({ requests, admin = false }: { requests: AddonRequest[]; admin?: boolean }) {
  return <div className="private-list">{requests.map((request) => {
    const service = optionalServices.find((item) => item.id === request.service_code)
    const monthly = request.addon_subscriptions?.find((sub) => sub.status !== 'draft' && (sub.activated_at || ['payment_pending','paid','scheduled','in_progress','completed','cancelled'].includes(request.status)))
    return <article key={request.id}>
      <div className="list-icon"><ServiceIcon name={service?.icon ?? 'mail'} /></div>
      <div><h2><Link to={admin ? '/admin/services/$id' : '/portal/services/requests/$id'} params={{ id: request.id }}>{service?.name ?? request.service_code}</Link></h2>
...
export function CustomerServices({ profile }: { profile: PortalProfile }) {
  ...
  useEffect(() => { void addonApi<{ payments: AddonPayment[] }>('payments').then((data) => setPayments(data.payments.filter((p) => !p.addon_request_id))).catch((error: Error) => setError(error.message)) }, [])
  return <PrivateShell area="portal" profile={profile} title="Services" eyebrow="Extra care, arranged personally">
    ...
    <section className="addon-history"><h2>Your requests</h2>{error && <p className="form-error" role="alert">{error}</p>}{loading ? <p role="status">Loading your requests…</p> : !error && requests.length === 0 ? <EmptyState title="No service requests yet">Your requests and their progress will appear here.</EmptyState> : <ServiceRequests requests={requests} />}</section>{payments.length > 0 && <section className="addon-history"><h2>Your payment arrangements</h2><CustomerPaymentRecords payments={payments} /></section>}
  </PrivateShell>
}
