...
export function AdminServiceAlerts() {
  ...
  useEffect(() => {
    void addonApi<{newRequests: AddonRequest[]; alerts: typeof alerts}>('admin/alerts')
      .then((data) => { setRequests(data.newRequests); setAlerts(data.alerts) })
      .catch((error: Error) => setError(error.message))
      .finally(() => setLoading(false))
  }, [])
  return <section className={`private-panel addon-alerts${requests.length ? ' has-new-requests' : ''}`} aria-labelledby="addon-alert-heading">
    <div className="addon-alert-heading"><span className="addon-alert-icon"><BellRing aria-hidden="true" /></span><div><p className="private-eyebrow">Action required</p><h2 id="addon-alert-heading">New Optional Service Requests</h2><p>{loading ? 'Checking for new requests…' : requests.length ? `${requests.length} ${requests.length === 1 ? 'request is' : 'requests are'} waiting for review.` : 'No new Optional Service requests.'}</p></div><span className="addon-alert-count" aria-label={`${requests.length} new service requests`}>{requests.length}</span></div>
    {error ? <p className="portal-notice error" role="alert">The new-request alert could not be loaded. {error}</p> : requests.length > 0 && <><div className="portal-notice warning" role="status"><span>Review each request before confirming its final payment amount.</span><Link to="/admin/services">Review all requests</Link></div><ServiceRequests requests={requests} admin /></>}
    {!error && !loading && requests.length === 0 && <Link to="/admin/services">View Services</Link>}
    {alerts.length > 0 && <div className="addon-payment-alerts"><h2>Payment alerts</h2><div className="private-list">{alerts.map((alert) => <article key={alert.id}><div><h3>{alert.label}</h3><p>{addonServiceCatalogue.find((s) => s.id === alert.payment?.service_code)?.name} · {alert.payment?.clients ? `${alert.payment.clients.first_name} ${alert.payment.clients.last_name}` : alert.payment?.customer_email} · {alert.payment?.properties?.display_name}</p><small>{date(alert.created_at)}</small>{alert.payment?.addon_request_id ? <Link to="/admin/services/$id" params={{id:alert.payment.addon_request_id}}>Open request</Link> : <Link to="/admin/add-on-payments">Open Add-on Payments</Link>}</div></article>)}</div></div>}
  </section>
}
...
export function AddonRequestDetail({ profile, id, admin = false }: { profile: PortalProfile; id: string; admin?: boolean }) {
  ...
  return <PrivateShell area={admin ? 'admin' : 'portal'} profile={profile} title={service?.name ?? 'Service request'} eyebrow={request?.request_reference}>
    <Link className="addon-back" to={admin ? '/admin/services' : '/portal/services'}>← Services</Link>{error && <p className="portal-notice error" role="alert">{error}</p>}{!detail ? !error && <p role="status">Loading request…</p> : <>
      <section className="private-panel"><dl className="addon-summary"><dt>Status</dt><dd><span className="private-pill">{admin && detail.request.status === 'paid' ? 'Payment processed' : addonStatusLabels[detail.request.status]}</span>{admin && detail.request.status === 'paid' && <small>Payment received. Operational completion remains separate.</small>}</dd><dt>Property</dt><dd>{detail.request.properties.display_name}</dd><dt>Requested</dt><dd>{date(detail.request.created_at)}</dd><dt>Published service price</dt><dd>{detail.request.published_price_snapshot}{detail.request.published_price_note_snapshot && <small>{detail.request.published_price_note_snapshot}</small>}</dd>{admin && <><dt>Customer</dt><dd>{detail.request.clients?.first_name} {detail.request.clients?.last_name}</dd><dt>Email</dt><dd>{detail.request.clients?.email}</dd><dt>Telephone</dt><dd>{detail.request.clients?.phone || 'Not provided'}</dd></>}</dl><h2>Additional details</h2><p className="addon-observations">{detail.request.customer_notes || 'No additional details provided.'}</p>{!admin && <p>A Guardemar agent reviews the details and confirms the final amount before payment. Sending this request does not confirm a booking.</p>}</section>
