...
import { addonPaymentStatusLabels, addonStatusLabels, shoppingPaymentMessage, type AddonPayment, type AddonRequest } from '@/lib/portal/addons'
...
export function adminPaymentStatusLabel(payment: AddonPayment) {
  const status = paymentDisplayStatus(payment)
  return payment.payment_type === 'one_time' && status === 'paid' ? 'Payment processed' : addonPaymentStatusLabels[status] ?? status
}
...
export function AddonPayments({ profile, requestId }: { profile: PortalProfile; requestId?: string }) {
  ...
  const linkedNeedsReview = linked?.status === 'requested'
  ...
  function selectRequest(id: string, list = requests) {
    ...
    setClientId(''); setFeeOnly(false)
    if (request) {
      setServiceCode(request.service_code); setEmail(request.clients?.email ?? ''); setDescription(`${addonServiceCatalogue.find((s) => s.id === request.service_code)?.name} — ${request.properties.display_name} — ${request.request_reference}`)
    } else {
      setServiceCode(''); setEmail(''); setDescription('')
    }
  }
  useEffect(() => {
  ...
  return <PrivateShell area="admin" profile={profile} title="Add-on Payments" eyebrow="Final amounts, personally confirmed">
    {options.livePaymentsEnabled
      ? <p className="portal-notice success" role="status">Secure Stripe Add-on payment links are active. Review every payment summary before sending.</p>
      : <p className="portal-notice" role="status">Secure Stripe Add-on payment links are unavailable because production billing is not configured.</p>}
    ...
    {profile.role === 'admin' && !review && <section className="private-panel"><h2>New payment</h2><form className="admin-form" onSubmit={prepare}>
      <label className="full-field">Optional Service Request reference <span>Select the customer request first. New requests remain visible but must be reviewed before payment.</span><select value={selectedRequest} disabled={pending} onChange={(e) => selectRequest(e.target.value)}><option value="">No linked request — External Provider Service only</option>{requests.filter((r) => ['requested','under_review','awaiting_customer'].includes(r.status)).map((r) => <option key={r.id} value={r.id}>{r.request_reference} · {addonStatusLabels[r.status]} · {r.clients?.first_name} {r.clients?.last_name} · {r.properties.display_name}</option>)}</select></label>
      {linked ? <label>Service<input readOnly value={addonServiceCatalogue.find((s) => s.id === linked.service_code)?.name ?? linked.service_code} /></label> : <label>Service<select value={serviceCode} required disabled={pending} onChange={(e) => { setServiceCode(e.target.value); setClientId(''); setEmail(''); setDescription(''); setFeeOnly(false) }}><option value="">Select a payment category</option>{addonServiceCatalogue.filter((s) => s.id === 'external-provider').map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label>}
      {external ? <label>Customer <span>Optional for email-only payments</span><select value={clientId} disabled={pending} onChange={(e) => { setClientId(e.target.value); setEmail(options.clients.find((c) => c.id === e.target.value)?.email ?? '') }}><option value="">Email-only customer</option>{options.clients.map((c) => <option key={c.id} value={c.id}>{c.first_name} {c.last_name}</option>)}</select></label> : linked && <label>Customer<input readOnly value={`${linked.clients?.first_name ?? ''} ${linked.clients?.last_name ?? ''}`} /></label>}
      ...
      {linkedNeedsReview && linked && <p className="portal-notice warning full-field" role="status"><span>This request is new. Review it and set it to Under review before preparing a payment.</span><Link to="/admin/services/$id" params={{ id: linked.id }}>Review request</Link></p>}
      {external && <p className="full-field">External Provider Service is recorded as a separate payment category. The amount entered is the exact final charge; no VAT is added on top and no 23% VAT breakdown is assumed.</p>}
      ...
      <div className="form-actions full-field"><button className="private-primary" disabled={pending || linkedNeedsReview || (!external && !linked)}>{pending ? 'Preparing…' : 'Review payment'}</button></div>
    </form></section>}
