...
function Services() { return <><StandardPageView pageKey="services" /><section className="section shell"><p className="eyebrow">Detailed service guides</p><h2 className="list-title">Understand the scope before you hand over access.</h2><div className="service-guide-grid">{Object.values(servicePages).map(service => <Link to={`/${service.slug}/`} key={service.slug}><span>{service.eyebrow}</span><h3>{service.title}</h3><p>{service.description}</p></Link>)}</div></section><section className="section warm-section"><div className="shell"><p className="eyebrow">On-demand services</p><h2 className="list-title">Practical help, arranged when needed.</h2><ul className="service-list">{services.map(service => <li key={service}>{service}</li>)}</ul></div></section></> }
