  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = await portalApi<{ inspection: { id: string } }>('admin/inspections', {
        method: 'POST',
        body: JSON.stringify({ propertyId: form.get('propertyId'), templateId: form.get('templateId'), inspectorStaffId: form.get('inspectorStaffId'), scheduledFor: new Date(String(form.get('scheduledFor'))).toISOString(), idempotencyKey: crypto.randomUUID() }),
      })
