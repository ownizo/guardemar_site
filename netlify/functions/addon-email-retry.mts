...
import { deliverAddonBillingEmail } from './_addon-email.mts'
...
export default async () => {
  ...
  const billing = checked(await database.from('addon_email_deliveries').select('email_key').is('sent_at', null).or(`first_attempt_at.is.null,first_attempt_at.gte.${new Date(Date.now() - 23 * 3600000).toISOString()}`).order('created_at').limit(5)) ?? []
  await Promise.allSettled(billing.map((item) => deliverAddonBillingEmail(database, item.email_key)))
